  ...
  <div class="user-info">
    ...
    <el-form label-width="220px">
      <div class="block">
        ...
        <el-form-item ref="mystaffphoto" label="员工照片">
          <!-- 放置上传图片 -->
          <UploadPhoto />
        </el-form-item>
...
import {getPersonalDetail,updatePersonal, saveUserDetailById} from '@/api/imployees'
export default {
...
methods:{
    async getUserDetailById(){
       ...
       if(this.userInfo.staffPhoto){
        this.$refs.staffphotos.fileList=[{url:this.userInfo.staffPhoto,upload: true}]
       }
   ...
   async getPersonalDetail(){
    ...
    async saveUser(){
      const fileList =this.$refs.staffphotos.fileList
      console.log(fileList)
      if(fileList.some(item => ! item.upload)){
        this.$message('此时还有图片未上传成功')
        return 
      }
